function [lambda, D_k1, N_k1] = compute_values_packet(k,beta,epsilon,N,var,a)
...
%define discontinuous kernel due to packet drop
    ...
    if k1==0
        ...
        M_k1 = Fie0_packet_drop(beta,epsilon,N, k1, kernel_packet, RHS_M, AbsTol, RelTol);
        L_k1 = Fie0_packet_drop(beta,epsilon,N, k1, kernel_packet, RHS_L, AbsTol, RelTol);
        
        D_k1 = L_k1 / M_k1;
        N_k1 = 1 / M_k1 - (1-beta);
    end

    M_k2 = Fie0_packet_drop(beta,epsilon,N, k2, kernel_packet, RHS_M, AbsTol, RelTol);
    L_k2 = Fie0_packet_drop(beta,epsilon,N, k2, kernel_packet, RHS_L, AbsTol, RelTol);
    
    D_k2 = L_k2 / M_k2;
    N_k2 =    1 / M_k2 - (1-beta);

   
    lambda = -(D_k2 - D_k1) / (N_k2 - N_k1+(1e-6));
